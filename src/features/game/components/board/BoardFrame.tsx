...
function Board({
  ...
  onSelect,
  onMark
}: BoardProps & { ids: Map<string, number>; snap: Move | null }) {
...
export function BoardFrame({ lastMove, isFlipped, ...board }: BoardProps) {
  ...
  useEffect(() => {
    const frame = snap === null ? null : requestAnimationFrame(() => setSnap(null))
    return () => {
  ...
  return (
    <div className='@container w-full select-none overflow-x-auto'>
      <div
        ...
        <Board {...board} ids={keys.ids} lastMove={lastMove} snap={snap} isFlipped={isFlipped} />
        <div />
