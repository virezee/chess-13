...
- "Watched" means any tile the enemy **attacks**, so that a piece standing there could take the Assassin on the following move, including the blast ring of an enemy Mage. A pinned enemy piece watches every tile it attacks, because watching asks where the enemy's attacks fall and not which of them the enemy would be allowed to play. This is the reading the Marshal's support takes as well.
